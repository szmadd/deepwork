 ...
 *
 * ── 场景矩阵 ─────────────────────────────────────────────────────────
 * | 编号 | DSH_PERMISSION_MODE | 目标位置   | 审批答复 | 用来回答什么 |
 * |------|---------------------|-----------|---------|-------------|
 * | A    | （不设 = 内核默认）   | 工作区内   | 拒绝     | **对照组**：这条指令真送到了、工具真跑了 |
 * | B1   | workspace-write      | 工作区外   | 拒绝     | 判据：边界挡不挡，以及挡在哪一层 |
 * | B2   | workspace-write      | 工作区外   | 放行     | 记录：用户点「允许」后，这次调用会怎样 |
 * | C    | read-only            | 工作区内   | 拒绝     | 判据：read-only 是否连工作区内也挡 |
 * | D    | danger-full-access   | 工作区外   | 放行     | **反证**：同一个目录在宽模式下写得进 → 差别只来自模式 |
 *
...
async function runScenario(dsh, home, base, name, opts) {
  ...
  record.toolStarted = Boolean(writeStart);
  record.toolRisk = writeStart?.call?.risk ?? '';
  const writeDone = events.find(
    (e) => (e.type === 'tool.completed' || e.type === 'tool.failed') && e.callId === writeStart?.call?.id,
  );
  record.toolOutcome = writeDone ? `${writeDone.type} ok=${writeDone.ok}` : '无工具终态事件';
  record.toolOutput = String(writeDone?.output ?? writeDone?.error ?? '').slice(0, 400);
...
function printRecord(r) {
  console.log(
    `    ${r.name.padEnd(22)} 模式=${String(r.mode).padEnd(18)} 审批答复=${r.approval.padEnd(5)}` +
      ` 写入=${r.written ? '是' : '否'} 审批请求=${r.approvals.length} run=${r.runDone}`,
  );
  if (r.toolOutcome !== '无工具终态事件') console.log(`      工具终态: ${r.toolOutcome}`);
...
async function main() {
  ...
  try {
    ...
    const B2 = by('B2');
    console.log(`  B2（workspace-write，工作区外，审批=放行）`);
    console.log(`    写入: ${B2.written}  审批请求数: ${B2.approvals.length}  工具终态: ${B2.toolOutcome}`);
    console.log(
      '    说明：按 dsh-fs-sandbox README，越界被拒后是「模型可获准以更宽模式重试同一操作」；',
    );
    console.log(
      '    替身端点不会自己重试，所以这里观察到的是「拦下之后停住」，而不是「批准即可写」。',
    );

    section('7) 解析器真帧自证：拒绝方言被接住了吗');
