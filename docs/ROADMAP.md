...
1. **端点模型清单拉取**：`GET /v1/models` 取端点真实模型 id，替代手输模型名；
   官方与自建端点同一条路径（官方目录以内核公布为准，见第 3 项）。
...
4. **连通性自检与降级**：填完端点先能"测一下"；端点不可达时如实提示，
   **不静默降级到 mock**（这与内核适配层的既有约定一致）。
...
**需求矩阵漏项**：有 4 条需求（FR-3.5 沙箱 / FR-3.8 图表 / FR-10.2 模型路由降级 / FR-10.5 崩溃上报）
**既不在 M2 剩余、也不在 M3** —— 本轮补录于第七节。规划下一步前先读那里。
...
connectors 42 / usage 27 / browser 76 / office 130 / **modelcfg 32**）；`real-dsh-mcp` 本机 3/8，已用 `git stash`
...
> **modelcfg 项数修正（2026-09-14）**：本表此前写 31，复跑实测 **32/32**
> （`8b7c487 fix(设置): 模型提供方切换改为本地状态` 之后新增了一项断言）。
> 这正是那条纪律的现场例证：**基线数字要来自当场那次命令的输出，凭印象写会全错而看不出来。**
...
| **模型选择与思考档** | **有**：`session/new` 的 `configOptions` 公布 `model`（select）与 **`reasoning_effort`**（off/low/high/max，默认 high） | **FR-10.2 的现成落点**：不必自建路由，先把内核已有这两个维度接出来（见第〇节第 2 项） |
...
| 编号 | 需求 | 优先级 | 现状 | 判据建议 |
|---|---|---|---|---|
| **FR-10.2** | 模型路由与降级策略（快模型 / 推理模型分工） | **P0** | 只有"提供方切换 + 自定义端点"；无路由、无降级。与第〇节第 2 项同一件事 | 契约里给出路由规则形状 → 实现 → `test:modelcfg` 扩充；**断言的参照物落在"实际发出的请求用了哪个模型"**，不是配置文本 |
...
**本轮（2026-09-14）建议动手顺序**（全部不依赖后端平台）：

1. **模型来源以内核/端点为唯一事实**（第〇节第 1+2+3 项 + FR-10.2）——
   起点是**取证**：跑 `tools/real-dsh-probe.js` 看官方模式下 `session/new` 到底公布了哪些模型，
   核对 `packages/core-host/src/models.ts` 的硬编码四条是否仍然一致。**先拿事实再动代码。**
