...
| **M2+ 收口后补强** | ✅ 100% | 模型目录以内核 `session/new` 真帧为准（删掉写死的四条）· 默认模型由用户自选 · 接出 `reasoning_effort`（2026-09-14 第二轮，见 DEVLOG） |
...
验证基线：`npm run verify` 18 套中 17 套全绿（diff / tools 19 / replay 29 / smoke 27 / partial 13 /
terminal 22 / acp 37 / real-dsh 15 / skills 59 / skillctx 24 / memory 38 / schedule 68 /
connectors 42 / usage 27 / browser 76 / office 130 / **modelcfg 80**）；`real-dsh-mcp` 本机 3/8，已用 `git stash`
...
> **modelcfg / smoke 项数修正（2026-09-14 第二轮）**：modelcfg 32 → **80**（模型目录解析层新增 48 条断言），
> smoke 26 → **27**（`models.list` 由「非空」升级为「目录带来源说明」，一条换两条）。
> 累计 **706 项通过**。基线数字必须来自当场那次命令的输出 —— 这条纪律上一轮已因 `modelcfg 31→32`
> 的 stale 偏差被写过一次，本轮继续逐项核对，未再发现偏差。
...
| 编号 | 需求 | 优先级 | 现状 | 判据建议 |
|---|---|---|---|---|
| **FR-10.2** | 模型路由与降级策略（快模型 / 推理模型分工） | **P0** | 🔶 **一半已落地（2026-09-14 第二轮）**：「模型 × 思考档」两个维度已接出（目录以内核真帧为准、默认模型用户自选、`reasoning_effort` 可设并实测到达端点）。**未做**：多模型自动路由（按任务挑模型）、端点不可达时的如实降级提示、云端/内网用量口径区分 | 剩余部分照旧：**断言的参照物落在"实际发出的请求用了哪个模型与哪一档"**（本轮已建立这套参照物：`tools/fixtures/openai-stub-llm.js` 的 `requests[].extra`） |
...
**下一轮建议动手顺序**（全部不依赖后端平台；第 1 项已于 2026-09-14 第二轮完成）：

1. ~~模型来源以内核/端点为唯一事实（FR-10.2 的前半）~~ ✅ **已完成**（2026-09-14 第二轮）。
   剩下的 FR-10.2 后半（自动路由 / 降级提示 / 用量口径）与本项同源，可接着做。
