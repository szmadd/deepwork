...
| 2 | 设置改左导航分组；管理类页面收进设置 | ✅ 完成（2026-09-18） | 设置页从「顶部三页签」改为**左导航 + 分组**，四组共 **12 节**：通用（外观 / 会话默认 / 界面与终端）、模型（模型与端点）、功能与数据（技能 / 记忆 / 自动化 / 连接器 / 用量）、安全与部署（审批与沙箱 / 部署与运行时 / 关于）。`AppView` 收窄为 `chat/files/terminal/browser/trajectory/settings`，五个管理页各自成为设置里的一节（面板加 `embedded` 内嵌模式：外壳由设置页给，页体一字不改）；输入框旁与顶栏的入口改成 `openSettings('skills'｜'usage')` 这样「进设置并定位到那一节」，**一页一个入口**。`tools/settings-nav-test.js` **35 项**进 verify |
