...
| 运维 | 已接：无。待补：**CI**（`npm run verify` 仍靠人工）、**远端推送**（`origin` 上 `main` 领先 **12** 个提交）、**版本 tag**（M0/M1 各阶段成果无回滚点） |
...
第八节做完后，ROADMAP 里「不依赖后端平台」的存量还剩两块，见 §七 与 §五：
FR-10.2 后半（自动路由与降级提示）、§五 的八项遗留债。
（**FR-3.5 的模式切换入口已于 2026-09-17 补齐**，见 §七 该行第三期。）
