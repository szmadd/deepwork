...
4. **运维债**（§五末行）：接 CI（**推远端、打 tag 已于 2026-09-17 落地**：`main` 已推、`v0.1.0` 已打）。
...
| 2 | 设置改左导航分组；管理类页面收进设置 | ✅ 完成（2026-09-18，同日两轮） | 设置从「顶部三页签」改为**左导航 + 分组**，四组共 **12 节**：通用（外观 / 会话默认 / 界面与终端）、模型（模型与端点）、功能与数据（技能 / 记忆 / 自动化 / 连接器 / 用量）、安全与部署（审批与沙箱 / 部署与运行时 / 关于）。五个管理页各自成为设置里的一节（面板加 `embedded` 内嵌模式：外壳由设置给，页体一字不改）；输入框旁与顶栏的入口改成 `openSettings('skills'｜'usage')` 这样「进设置并定位到那一节」，**一页一个入口**。同日第二轮按用户要求把设置本身从**整页**改成**覆盖层对话框**（880×600，✕ / Esc / 点遮罩三条退路），`AppView` 随之再收窄一次、`settings` 退出视图清单（「设置开着没有」不落盘，「停在哪一节」落盘）。`tools/settings-nav-test.js` **46 项**进 verify |
