...
用户看到整页版之后当场提的下一条：**设置本身改成弹出式（覆盖层），比整页小一圈、更紧凑。**
「管理类不做弹窗」这条旧判据要按**具体缺陷**重读，而不是按「弹窗」这个词 —— 见下一篇。


## 2026-09-18 · M3 前置 · 设置改成覆盖层：从「又一页」变成「盖上去的一层」

**目标**

用户看到上一轮的整页版之后提的是：**设置改成弹出式，界面缩小，比现在好看紧凑。**
上一轮刚把管理类收进设置、设置长到 12 节；这一轮把它的**形态**换掉 ——
不再占满主区，改成居中的覆盖层对话框。

**现象与根因**

把设置当成「主区里的一个视图」有两处不成立：

1. **它不占主区里一个可以停下来的位置。** 文件 / 终端 / 轨迹是「会在那里停一会儿」
   的地方（翻目录、连着敲命令、顺着时间线往回看），设置不是 —— 专门去配一次，
   配完关掉，回到进来之前那一页。它盖在所有页上面，自己不留位置。
2. **「上次开着设置」不是一种该被恢复的状态。** 而 `lastView` 是落盘的：上一版里
   关掉应用时停在设置页，下次启动就会 `lastView: 'settings'` → 主区**什么都不渲染**
   （`view === 'settings'` 那个分支一删，就没有任何东西接住它），而 rail 还会把高亮
   打在一个主区里不存在的页面位置上。

而这里还压着一条**上一轮自己写下的相反判据**：「管理类不做弹窗，因为可用面积被压到
620px 宽、且背景『看起来还在、其实点不到』」。这条不能悄悄绕过 —— 它是**那个弹窗**的
两条具体缺陷，不是浮层这种形态的错。所以本轮把判据重写成一句可判的：
**它占不占主区里一个可以停下来的位置**，并列出四种形态各自的位置：

| 形态 | 谁 | 尺寸 | 语义 |
|---|---|---|---|
| rail | 工作台五项 + 底部设置按钮 | 56 / 148px | 天天点 |
| 整页（`PanelPage`） | 文件 / 终端 / 轨迹 | 占满主区 | 会停一会儿，常要一边看一边回对话 |
| 覆盖层（`.settings-mask`） | 设置 | 880×600 | 专门配一次，关掉回到进来那一页 |
| 打断式弹窗（`.modal-mask`） | 审批 / 只读预览 | 620 / 920px | 内核停下来等你拍板 |

**改动**

| 位置 | 内容 |
|---|---|
| `packages/protocol/src/config.ts` | `APP_VIEWS` 去掉 `settings`（只剩五个工作台视图），`APP_VIEW_LABEL` 同去；`SETTINGS_SECTIONS` 的注释补上「设置是覆盖层，所以这一层只回答停在哪一节」；`AppConfig.settingsSection` 的注释改写为「两个状态必须分开：开着没有不落盘，停在哪一节落盘」 |
| `packages/core-host/src/host.ts` | `setConfig` 的视图拒绝文案补上「设置本身是覆盖层，也不再是一种视图」；`getConfig` 折回的注释补上第二种旧值（上一版可能存着 `"lastView": "settings"`）。折回逻辑本身不用改 —— 它按白名单判，白名单少一个值就自动多拦住一个 |
| `apps/desktop/src/components/SettingsPanel.tsx` | 外壳从 `page-mask` / `page` 换成 `settings-mask` / `settings-dialog`（顶部条只放「设置」+ ✕）；删掉页脚那条「返回对话」；注释里正面回答上一版「为什么不做弹窗」 |
| `apps/desktop/src/components/ActivityRail.tsx` | props 加 `settingsOpen` / `onOpenSettings`；`ICONS` 表去掉 `settings` 项，改成独立的 `SETTINGS_ICON` + 自己一段 JSX（那张表的键是 `AppView`，而设置不是视图了）；点亮改看 `settingsOpen` |
| `apps/desktop/src/App.tsx` | 新增 `settingsOpen` 临时状态（不落盘）+ Esc 监听（只在开着时挂）；`openSettings` 不再写 `lastView`、也不再 `setView`；覆盖层移到 `main` 之外、挂在审批弹窗**前面**；布局契约注释加第 5 条「两层浮层，语义不同，不要合并」 |
| `apps/desktop/src/components/PanelPage.tsx` | 注释整段改写：从「为什么功能页不再是居中弹窗」改成「什么该是整页、什么该是浮层」，并说清旧理由是按**具体缺陷**读、不是按词读 |
| `apps/desktop/src/components/SettingsNav.tsx` | 未改（它本来就只认契约，这一轮正好验证了那份抽象的成色） |
| `apps/desktop/src/styles.css` | 新增 `.settings-mask` / `.settings-dialog` / `.settings-dialog-head` / `.settings-close`；两栏整体收紧（导航 168→152px、条目 12.5→12px、内容列内边距 14/20/24→12/18/20）；节标题下压一根淡分割线；输入控件 `max-width: 520px`（690px 宽的一行输入框读起来是散的）；顺手修掉一处（见下） |
| `tools/settings-nav-test.js` | 35 → **46 项**：视图清单从六个收到五个、新增「设置不是视图也不是分节」、新增「启动视图下拉里没有设置」、新增「`settings` 作为视图被拒绝」与「旧文件里的 `settings` 被折回」、新增覆盖层的四条接线断言 |
| `tools/capture.sh` | `settings` 场景改聚焦 `.settings-dialog` 并回读尺寸；新增 `settings-close`（Esc 真的关掉，回读对话框与遮罩都归零）与 `settings-dark`（深色下两栏底色不能是同一个色，回读两个 computed 值） |

**顺手修掉的一处真缺陷：导航列与它的悬停底色是同一个色**

`.settings-nav` 这一轮加了 `background: var(--bg-2)`（让两栏靠底色分层），
而 `.settings-nav-item:hover` 原本就是 `var(--bg-2)` —— 于是在浅色主题下，指针移到条目上
**什么都没有发生**，看起来像「这栏就是不响应」。上一轮刚新增的那条断言（把样式表里
所有 `var(--x)` 抓出来逐个核对定义）挡不住这一种：两个变量**都定义了**，
问题在「同一个色值被用在两层」。这一条只能靠**看**，所以本轮给深色也补了一场截图，
并把两个底色回读出来比对（`dialog-bg:rgb(27,29,33) nav-bg:rgb(33,36,41) same:no`）。

**验证**

```
node tools/settings-nav-test.js   # 46/46 通过
npm run typecheck                 # protocol / core-host / desktop 三处全过
npm run verify                    # exit 0 —— 31/32 绿 + 1 个已知环境性（browser）
bash tools/capture.sh settings settings-close settings-dark settings-prefs \
                      skills memory usage
  ui-settings       → items:12 on:模型与端点 groups:通用/模型/功能与数据/安全与部署
                       dialog:880x600 mask:1544x921
  ui-settings-close → dialog:1->0 mask:0 closed          ← Esc 真的关掉了
  ui-settings-dark  → theme:深色 dialog-bg:rgb(27,29,33) nav-bg:rgb(33,36,41) same:no
  ui-settings-prefs → model:(跟随) options:2 effort:2
  ui-skills / ui-memory / ui-usage → 三个内嵌面板在 600px 高的对话框里排版正常
                                      （用量那张图能整块放进内容列，这是尺寸够用的硬证据）
```

`dialog:880x600 mask:1544x921` 这三段数字是本轮最该留下的一条回执：
**图上看不出「对话框是 880 宽还是撑满了窗口」**（窗口本来就 1544 宽），
而遮罩的 1544×921 = 整个窗口，才是「背景确实盖住了 rail 与会话列表」的证据。

**踩坑与教训**

1. **判据要写成「可判的一句话」，不要留在「弹窗」「整页」这类词上。** 上一轮的
   「管理类不做弹窗」看着像原则，其实是两条具体缺陷（620px 太窄、背景看着还能点）。
   词一旦被当成判据，下一次需求变化时就会有人（包括我自己）在「一条原则」和
   「用户要的东西」之间二选一 —— 而正确的动作是回头查那条原则当年在防什么。
   这轮把判据换成「它占不占主区里一个可以停下来的位置」，四种形态各自能对照检查。
2. **一个状态该不该落盘，取决于它是不是「用户下次还想看到的东西」。** 「设置开着没有」
   落盘的后果是开机弹窗；「停在哪一节」不落盘的后果是每次重找。两者在同一份配置里、
   名字也像，但答案相反 —— 所以 `settingsSection` 的注释现在把这句话写在最前面。
3. **断言要盯渲染出来的东西，不要盯文件里出现过这几个字。** 新增的「设置不再借整页外壳」
   第一次跑就红了：`page-mask` 确实还出现在 `SettingsPanel.tsx` 里 —— 在一句**注释**中
   （说明旧外壳为什么不在）。把注释也算违规的断言，只会让下一次补说明的人莫名其妙地
   去改注释。改成盯 `className="page-mask"` 之后才是它真正想验的那件事。
4. **两个变量都定义了、但用在两层是同一个色 —— 这一类只有眼睛能抓。** 详见上面那处
   真缺陷。它与「变量未定义」（断言能抓）是同一族问题的两半，两半都要有分工。

**遗留**

- **设置里没有搜索**（上一轮就记着）：12 节之后找一项要按分组逐节翻。弹出式之后这个问题
  更明显了 —— 对话框小，一屏能看到的条目更少。
- **`settingsSection` 仍没有「每次打开都回到某一节」的偏好项**，只有「上次停在哪一节」。
- **对话框尺寸是写死的 880×600**（窗口更小才跟着缩）。小屏上「偏大」、超宽屏上「偏小」
  都可能出现；要不要按窗口分档，等真实反馈。**没有任何断言在守这个尺寸**，
  它只活在截图回执里。
- **`.settings-tabs` 仍在使用**（记忆面板的三层切换）—— 两套「切换」控件并存，下次留一眼。
- **流式输出仍未做**（用户排第三）。内核 ACP 桥只转发**已提交**的整块消息，没有 delta
  级事件；真正的 token 级流式要走 `dsh-client-connection`（需 dsh Host + HTTP/WS）。
