...
| **需求矩阵漏项**（ROADMAP §七） | FR-10.2 模型路由与降级 · FR-3.5 沙箱 · FR-3.8 图表 · FR-10.5 崩溃上报 | 🔶 进行中 | FR-10.2 主干已落地（第二 / 三 / 四轮）；**FR-3.5 三期全部完成**（第五轮取证确认内核本就装配沙箱 + 接出生效口径；第六轮真内核端到端证明该口径真的约束模型写文件 + 拒绝在界面上说人话；**2026-09-17 尾项补齐档位切换入口** —— 设置页三档选择 + 重启内核，`sandbox-test` 41 → 62 项）；**FR-3.8 图表已完成**（2026-09-16）。剩：模型升级路径取证（模型被拒后真会重试吗）、FR-10.2 的自动路由 |
...
5. **整链顺序里 `connector-test` 崩过一次（非本轮回归，如实记）。** 顺序跑 24 套时，
   `connector-test` 在 `schedule-test` 之后抛出未捕获异常（tail 只剩一个 `}` 与 `Node.js v22.22.2`）。
   随后**单跑 3 次 + 按批量顺序（`schedule-test` 之后）1 次，全部 42/42**，
   共 5 连绿，**未能复现**。本轮改动不触及连接器路径（只加了宿主构造期的配置读取），
   且失败形态是未捕获异常而非断言失败 —— 归为顺序/资源相关的偶发，
   **没有把「它通过了」写成结论**：下次整链再出现时应当场把完整栈留下来。
