...
- **分叉点按事件 seq 精确切，不吸附到 run 边界**（M1 遗留「逐事件分叉」，2026-09-17）。落在半轮里就切在半轮——因为继承的是**记录**（会话日志），不是模型的上下文，半步既不会出错也不该被拒。仅当请求的 seq 不在日志里时，才取「不晚于它的最近事件」（`atSeq` 与 `requestedSeq` 的差异**只**表示这件事）。
- 新会话（只有一条 `session.created`）是合法切点，可精确继承这 1 条；切在首条事件之前才是非法位置（`没有可继承的事件`）。
- 日志是复制而非引用，删父会话不影响分支。
...
### 界面布局纪律（M2-J 起；2026-09-18 两次收窄）

**判据只有一条：它占不占主区里一个可以停下来的位置。**

| 形态 | 谁 | 尺寸 | 语义 |
|---|---|---|---|
| 活动栏（rail） | 对话 / 文件 / 终端 / 浏览器 / 轨迹 + 底部一个设置按钮 | 56 / 148px | 工作台，天天点 |
| 整页（`PanelPage`） | 文件 / 终端 / 轨迹 | 占满主区 | 会在那里停一会儿，且常要一边看一边回对话 |
| 覆盖层（`.settings-mask`） | 设置 | 880×600（窗口更小就跟着缩） | 专门去配一次，配完关掉回到进来那一页 |
| 打断式弹窗（`.modal-mask`） | 审批 / 只读预览 | 620 / 920px | 内核停下来等你拍板，你只能先处理它 |

**活动栏只放工作台视图，管理类收进设置。** 工作台 = 对话 / 文件 / 终端 / 浏览器 / 轨迹
（就是 `APP_VIEWS` 的全部），加固定在底部的设置按钮。技能 / 记忆 / 自动化 / 连接器 / 用量
**不是视图**，是设置里的分节（`SETTINGS_SECTIONS`），入口只有「设置 → 对应那一节」。
理由是使用频率差一个数量级：把「天天点的」和「偶尔来配一次」放进同一根栏，
栏会随着功能增加一直变长，而它本来是为了「第 10 个功能与第 1 个占用同样空间」才存在的。

**设置自己也不是视图 —— 它是覆盖层，`APP_VIEWS` 里没有 `settings`。** 判据是同一句：
它不占主区里一个可以停下来的位置，它盖在所有页上面，关掉就回到原处。于是两个状态必须分开：
「设置开着没有」是渲染层的临时状态、**不落盘**；落盘的只有「上次停在哪一节」
（`config.settingsSection`）。把前者写进 `lastView` 的后果很具体：下次启动弹一个对话框出来，
而用户的第一动作是关掉它。rail 上那个设置按钮也因此**不走 `ICONS` / `item()` 那条路径**
（那张表的键是 `AppView`，混进去「栏上有几个图标」与「栏上该有几个图标」就对不上了），
它自己一段 JSX，点亮靠 `settingsOpen` 而不是 `view === 'settings'`。

**「弹窗不好」这条旧理由要按具体缺陷读，不要按词读。** 这里曾写着「管理类不做弹窗」，
理由有两条：可用面积被压到 620px 宽、背景「看起来还在、其实点不到」。那是**那个弹窗**的
两条具体缺陷，不是浮层这种形态的错 —— 设置现在 880×600，遮罩整块盖住 rail 与会话列表，
背景明确地不可点，而用户要的恰恰是「它像一层盖上去的东西，而不是又一个页面」。
判据始终是上表那句话，不是「弹窗」「整页」这两个词本身。

**新增一个功能页 = 先回答它是工作台、设置，还是浮层**：工作台 → `APP_VIEWS` 加一项 +
`APP_VIEW_LABEL` 加一行 + rail 加一个图标 + 主区加一个分支；设置 → `SETTINGS_SECTIONS`
加一项 + `SETTINGS_GROUPS` 里挂到某个分组 + 加一块内容（面板记得支持 `embedded`）；
浮层 → 自己的遮罩 + 由 DOM 顺序决定与既有浮层的层级关系。
`tools/settings-nav-test.js` 会挡住「契约收了、rail/导航忘了改」——
这类改动的失败形态全是静默的：漏掉的那一节在设置里**永远打不开**，
而它的面板代码、配置项、测试全都还在。

**一页一个入口。** 技能 / 用量这类面板只有设置里那一个挂载点；
输入框工具行与顶栏上的按钮不另开页面，而是 `openSettings('skills')` 这样
「进设置并定位到那一节」。两个入口、两套外壳的代价是同一个缺陷只在其中一个入口可见 ——
而用户不会两个都试一遍。这条路曾经真的走偏过：技能既是 rail 上的 `skills` 视图、
又是输入框旁边的按钮，两边各自渲染外壳。

**覆盖层必须给三条退路，缺一条用户就会先怀疑自己点错了**：右上角 ✕、Esc、点遮罩。
Esc 挂在 `window` 上（焦点可能停在任何输入框里）且**只在开着时才挂** —— 常驻监听会让
「关掉」这个动作在关着的时候也执行一遍，而用户看不出来是谁做的。点遮罩用 `onMouseDown`
而不是 `onClick`：后者会把「在对话框里按下、拖到遮罩上松开」判成点击遮罩，
而那个手势的真实意图通常是「我在选文字，松手位置偏了」。

**浮层之间的关系由 DOM 顺序定，不靠调 z-index。** 审批与设置同 z-index（40），
设置挂在审批**前面** —— DOM 靠后的赢。理由不是排版：审批是「内核停下来等你拍板」，
它不该被一层设置挡住。

**管理类面板支持 `embedded`。** 面板的整页外壳（`.page-mask` / `.page-head` /
页脚里的「返回对话」）在设置里是重复的，所以嵌进去时由面板自己收掉外壳、
只交出页体（见 `panel-embed`）。**页体一字不改** —— 任何「嵌进来时少显示一点」
的写法都会让同一个缺陷只在其中一个入口可见。同样的道理适用于 `.settings-tabs`：
它留给记忆面板的分层用（同一层里切三种东西），不再用来切「设置的大节」。

**视图落盘在 `config.lastView`，设置分节落盘在 `config.settingsSection`。**
`getConfig` 按字段合并默认值，旧 config.json 少这两个键就走默认，不需要迁移脚本。
渲染层不自己记「上次在哪」，否则与 config 各说一套 —— 恢复时的那条竞争（配置异步到达，
把用户刚点的那一页顶掉）在 `App` 里用「只恢复一次 + 用户动过就不再覆盖」处理，
两个键各自一套 ref。**第三样状态是「设置开着没有」，它不落盘**（原因见上面那条）。

**契约值域收窄时，读的路径要折回、写的路径要拒绝。** `AppView` 两次去掉值
（`skills` 等五个 → 设置里的分节；`settings` → 覆盖层）之后，一台升级上来的机器
`config.json` 里可能还存着其中任一个：原样交给渲染层，下次启动会停在一个没有对应页面的
视图上 —— 主区一片空白，而原因只写在配置文件里。所以 `getConfig` 折回合法值
（并且在日志里点名），`setConfig` 直接拒绝并在错误里说清「它去哪了」。
两个方向都要有：只折回不拒绝 = 界面自己可以写出坏值；
只拒绝不折回 = 手改过的旧文件让界面停在不存在的页面上。

**图标是内联 SVG，不引图标库。** 与「装完就能跑」同源：多一个依赖就多一份体积与供应链面。
设置里的左导航是纯文字（分组 + 条目），因为多了图标它就要挤占宽度，而那一列只有 152px。

**面板只渲染，不决定何时重新拉数据。** 刷新时机统一在 `App` 的 `refreshTarget`
（一张表同时覆盖视图与设置分节）与事件回调里 —— 否则每个面板会长出自己的刷新策略，
出现「同一份数据在不同页面上新旧不一」。视图名与分节名的取值域不相交，
所以合成一张表就够；分成两份的理由只能是「它们是两件事」，而它们不是。
...
- **写入先于回复。** 三条路径：UI 面板显式写入、`run` 结束后宿主向当日日志追加一行
  （时间/输入前 80 字/结果）、**内核经 MCP 自主写记忆**（M2-E 已落地，2026-09-17）。
- **内核写记忆走独立 MCP 服务，与面板共用同一个 `MemoryStore`。** 服务名 `deepwork_memory`，
  工具 `memory_write` / `memory_read`，形态与 chart / browser 服务一致（`buildMemoryMcpPatch`，
  注入顺序 chart → memory → browser）。**单一事实来源**：内核写进磁盘的那份，与面板读的是同一份，
  不存在「两套记忆」。
- **画像（profile）对内核写禁。** `MEMORY_WRITE_LAYERS = ['user','workspace']` —— 画像只由用户
  经 `memory.setProfile` 维护。内核写入复用 `MemoryStore.add` 的**预算闸**（超限抛可行动错误），
  不另起一套。写入条目 `origin: 'agent'`（用户写的仍是 `'user'`）。
- **日志 append-only。** 每日日志只有追加这一个写入口（`appendDailyLog`），
  唯一的「移动」是归档：mtime 超 30 天的日记在读取侧惰性按月并入
  `archive/YYYY-MM.md` 后删原文件。**归档是机械合并，不是语义蒸馏** ——
  蒸馏需要内核摘要能力，届时以 `origin: 'distilled'` 条目回写精选层。（**去重仍未做**。）
...
### 判源纪律（技能 URL 安装源，M2-C）

- **按内容判源，不按后缀猜。** `classifySkillSource` 先看字节：PK 头 ⇒ zip；
  文本且以 `---` frontmatter 开头 ⇒ SKILL.md；都不像就如实拒绝并说明**看到的是哪种字节**。
  后缀名（`.zip` / `.md`）不可信，不作为判据。
- **scheme 判定要覆盖 Windows 盘符。** `file://` / `ftp://` 是**非法安装源**（给可行动提示），
  不能被误判成 `local-dir`（否则报「目录不存在」这种误导错误）；`C:\...` 是本地目录不是协议 ——
  用 `长度===1 的单字母 scheme ⇒ 本地` 这条规则处理。
- **zip 解压是攻击面，必须逐项设防。** 手写零依赖解压器：拒绝路径穿越 / 绝对路径 / 盘符 /
  符号链接条目 / 异常压缩方法 / zip64 / 条目数与解压体积炸弹。任一命中即整包拒绝，不静默跳过。
- **URL 安装与本地安装同一条路径。** `installFromUrl` 必须先过**同一套审计**再落盘，源 URL 记进
  manifest（可追溯）；404 / 网络失败**返回结果对象**（带如实原因），不抛未捕获异常。
- **审计白名单未做**：误报无法标「已知合法」——这是 §五 留档的未做项，别在文档里写成已支持。

### 主题与通知纪律（2026-09-17）

- **`data-theme` 挂 `document.documentElement`，不是 `body`。** 原生控件（滚动条 / 表单 / 弹窗）
  跟随 `color-scheme`，挂错节点会出现「自绘部分变深、原生部分还是浅」的半吊子状态。
- **主题变量必须成对补齐。** 新增语义色时，`:root` 与 `html[data-theme='dark']` 两处都要覆盖；
  theme-test 有「每个 `:root` 变量在深色下都有覆盖」的完整性断言（共享变量白名单见测试）。
- **默认主题是 `light`。** 曾默认 `dark` 但字段从未被消费 —— 接切换器前必须先确认真实默认值，
  否则一上线就静默翻转整个 UI。
- **通知判据：`!(windowVisible && isCurrentSession)`。** 应用在前台**且正是当前会话**时不打扰。
  `shown` 的语义是「已交给系统」，**不是**「用户看见了」—— UI 文案不得写成后者。
- **通知文案对整句裁剪**（标题 ≤60 / 正文 ≤160），不是只裁变量。`run.notice` 只对 `warn` 出通知。
...
npm run verify          # 差异引擎随机对拍 + 写工具守卫 19 项 + 回放/分叉 29 项
                        ...
                        # + 模型路由与端点降级 39 项（FR-10.2 后半起新增：失败分类七种（真发 HTTP，
                        #   本机假端点）/ 可达性缓存与新鲜期 / 提示措辞（「不可达」与「连上了」不许混）/
                        #   宿主调用点 / 按会话模式指定模型 / 界面接线）
                        # + 技能 URL 安装源 47 项（M2-C 遗留起新增：按内容判源 / scheme 与盘符判定 /
                        #   zip 安全（路径穿越·绝对路径·符号链接·异常方法·zip64·体积·条目数）/ 真 HTTP 端到端）
                        # + 主题切换 24 项（界面遗留起新增：resolveTheme / 默认 light / CSS 变量深色全覆盖 / 宿主 config 闸）
                        # + 桌面通知 29 项（M2-F 遗留起新增：判据三态 / 整句裁剪 / 事件覆盖 / 通知通道与横幅接线）
                        # + 分叉与分支对比 30 项（M1 遗留起新增：两来源取最后变更 / 精确分叉 / 共享工作区 / RPC 与面板接线）
                        # + Composer 技能补全 29 项（M2-D 遗留起新增：触发条件 / 与内核 EXPLICIT_RE 对齐的插入 / 三处正则一致）
                        # + 真实 dsh+MCP 端到端 8 项（M2-G 起新增：真实插件加载；**已知环境性、由编排器点名**）
...
> **「跑完」与「判红」是两件事（2026-09-17 起 `verify` 走 `tools/verify-all.js`）**：
> 旧 `verify` 用 `&&` 串联，任一套件 `exit ≠ 0` 就**中断整条链**。本机有两个**已知环境性失败**
> （`browser-test`：沙箱内 Electron 起不了 Edge；`real-dsh-mcp`：真实 MCP 插件未注册工具）。
> 只把「一个」已知会失败的放链尾并不能解决问题 —— 两个环境性失败没法同时放链尾，
> 先出现的那个会把后面的全部掐掉。实测代价：`browser-test` 排在 office/chart/… 之前，
> 导致**其后 15 个套件从未在本机 verify 里被执行过**，而外层看起来像「跑到这儿都过了」。
> 现在 `verify` 由编排器跑**全部**套件再汇总：非环境性套件全绿 ⇒ `exit 0`；
> 已知环境性失败单独点名（`🟡`）但不判红。要旧的快速失败行为用 `npm run verify:strict`。
> **新增套件时按顺序加进 `tools/verify-all.js` 的 `SUITES` 与 `verify:strict` 两处，别只改一处。**
...
> 旧记录保留（它记的是一次真实观测）。
> **2026-09-17 补记（第三次观测）**：又复现 3/8，且这次**当场做了改动前后的对照** ——
> `git stash push -u` 回到 HEAD 干净树、重建、连跑三次，**每次都是 3/8**，与本轮八项改动无关。
> 失败形态稳定（握手 ok、run completed、`mcpCapabilities` 正常，但模型工具表里**无任何 `mcp__`**）。
> 这与上面「两次观测都改动前后一致，而一致的结果本身在变」完全吻合：**它随机器状态漂**。
> 判回归的唯一合法做法仍是**改动前后各跑一次比**。
>
