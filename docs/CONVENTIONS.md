...
npm run verify          # 差异引擎随机对拍 + 写工具守卫 19 项 + 回放/分叉 29 项
                        ...
                        # + 沙箱后端 41 项（FR-3.5 起新增：runner 真帧对照/模式解析/内核装配真帧/
...
> **`real-dsh-mcp` 的「本机 3/8」已不再复现（2026-09-16 复测，两次）**：这一轮在**改动前的
> 干净树**（`615c394`）与改动后的树上各跑一次，两次都是 **8/8 全绿**，`mcp__fake__echo`
> 正常出现在内核工具列表里。也就是说 2026-09-13 记的那 5 项失败，在今天的机器上不成立。
> 它可能是当轮的网络/沙箱状态，也可能被后续某次改动顺带修掉了 —— **归因未定，不要当结论**。
> 这条旧记录保留（它记的是一次真实观测），但**不要再拿「本机 3/8」当基线引用**；
> 引用之前先跑一次 `node tools/real-dsh-mcp-test.js`。
>
