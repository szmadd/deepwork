...
### 界面布局纪律（M2-J 起；2026-09-18 收窄入口）

**活动栏（rail）只放工作台视图，管理类收进设置页。** 工作台 = 对话 / 文件 / 终端 /
浏览器 / 轨迹（`APP_VIEWS` 前五项），加固定在底部的设置。技能 / 记忆 / 自动化 /
连接器 / 用量**不是视图**，是设置页里的分节（`SETTINGS_SECTIONS`），入口只有
「设置 → 对应那一节」。理由是使用频率差一个数量级：把「天天点的」和「偶尔来配一次」
放进同一根栏，栏会随着功能增加一直变长，而它本来是为了「第 10 个功能与第 1 个占用
同样空间」才存在的。

**新增一个功能页 = 先回答它是工作台还是设置**：工作台 → `APP_VIEWS` 加一项 +
`APP_VIEW_LABEL` 加一行 + rail 加一个图标 + 主区加一个分支；管理类 → `SETTINGS_SECTIONS`
加一项 + `SETTINGS_GROUPS` 里挂到某个分组 + 设置页加一块内容（面板记得支持 `embedded`）。
`tools/settings-nav-test.js` 会挡住「契约收了、rail/导航忘了改」——
这类改动的失败形态全是静默的：漏掉的那一节在设置页里**永远打不开**，
而它的面板代码、配置项、测试全都还在。

**一页一个入口。** 技能 / 用量这类面板只有设置页里那一个挂载点；
...
页脚里的「返回对话」）在设置页里是重复的，所以嵌进去时由面板自己收掉外壳、
...
旧的 `sidePanel`（none/tree/terminal）已被前者取代；`getConfig` 按字段合并默认值，
旧 config.json 少这两个键就走默认，不需要迁移脚本。渲染层不自己记「上次在哪」，
否则与 config 各说一套 —— 恢复时的那条竞争（配置异步到达，把用户刚点的那一页顶掉）
在 `App` 里用「只恢复一次 + 用户动过就不再覆盖」处理，两个键各自一套 ref。

**契约值域收窄时，读的路径要折回、写的路径要拒绝。** `AppView` 去掉五个值之后，
一台升级上来的机器 `config.json` 里可能还存着 `"lastView": "skills"`：原样交给渲染层，
下次启动会停在一个没有对应页面的视图上 —— 主区一片空白，而原因只写在配置文件里。
所以 `getConfig` 折回合法值（并且在日志里点名），`setConfig` 直接拒绝并在错误里
说清「它去哪了」。两个方向都要有：只折回不拒绝 = 界面自己可以写出坏值；
只拒绝不折回 = 手改过的旧文件让界面停在不存在的页面上。

**图标是内联 SVG，不引图标库。** 与「装完就能跑」同源：多一个依赖就多一份体积与供应链面。
设置页的左导航是纯文字（分组 + 条目），因为多了图标它就要挤占宽度，而那一列只有 168px。
