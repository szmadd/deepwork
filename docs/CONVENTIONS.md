...
> **已知的环境性失败（2026-09-13，M2-J 轮次）**：`real-dsh-mcp` 在本机当前网络/沙箱下 3/8 通过
> （`mcp__fake__echo` 没有出现在内核工具列表里；`start()` 仍报 mcp 能力加载成功）。
...
>    **不是** `real-dsh-chart` 补丁注入引起（该套件根本不构造 runtime patch）。
